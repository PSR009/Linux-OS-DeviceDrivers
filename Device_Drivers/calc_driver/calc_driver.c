...
static ssize_t test_read(struct file *filep, char *k[2], size_t len, loff_t *off)
{
    printk("\nIn test_read\n");
    copy_to_user(k[2], r, sizeof(int));
    return len;
}

static ssize_t test_write(struct file *filep, const char *k[2], size_t len, loff_t *off)
{
    printk("\nIn test_write\n");
    copy_from_user(s[2], k[2], 2*sizeof(int));
    r = k[0] + k[1];
    s[2] = {0};
    return len;
}
